...
from ok.hy.rag_code import Transcribe, EmbedData, QdrantVDB_QB, Retriever, RAG
...
if prompt := st.chat_input("Ask about the audio conversation..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            streaming_response = query_engine.query(prompt)  # Check if this is streaming or a complete response

            if isinstance(streaming_response, str):  # If it is a complete response
                full_response = streaming_response
                message_placeholder.markdown(full_response)
            else:  # Assume it's streaming
                for chunk in streaming_response:
                    try:
                        ...
                        break  # Stop looping if there's an issue

            message_placeholder.markdown(full_response)  # Final response update

        except Exception as e:
            st.error(f"Error in query processing: {e}")
